...
object consists of internal data and methods that preform operations on the data. <br/>
When an existing object type doesn't fully suit your needs you can create a **class**. <br/>
**Inheritance** - you can define a new object type aka a new class that inherits properties from an existing object type. <br/>
`class MyList(list):`
class created by **Inheritance**. The new class inherits the attributed defined by its base class, the class it's inheriting attributes from (like list here). The derived class "MyList" can both redefine any of the inherited attributes and can also add its own new attributes. Class statement sets up a blue print of a new class including what methods and operations they support, but does not create any object of that type. When a object of a particular type is created that object is an "instance" of that type. The class statement doesn't create any instances of the class.
```
class MyList(list):
    def remove_min(self):
...
```
The name of the class is called "self" and is always passed as the first argument to the functions defined as part of a class. Two functions are defined as part of a class. Two functions, two instance methods where remove_min and remove_max is called.
<br/>
`x = [1,2,3,4,5,6,7,8,9]` <br/>
`y = MyList(x)` #makes a copy of x with the added methodes defined in MyList
